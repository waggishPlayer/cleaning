...
const { user, logout } = useAuth();
  ...
  const stats = {
    admin: [
      { label: 'Total Users', value: '1,234', color: 'bg-black', icon: '👥' },
      { label: 'Active Bookings', value: '87', color: 'bg-black', icon: '📅' },
      { label: 'Total Revenue', value: '$12,450', color: 'bg-black', icon: '💰' },
      { label: 'Workers Active', value: '23', color: 'bg-black', icon: '🧑‍💼' }
    ],
    user: [
      { label: 'Total Bookings', value: userStats.bookings.toString(), color: 'bg-black', icon: '📅' },
      { label: 'Vehicles', value: userStats.vehicles.toString(), color: 'bg-black', icon: '🚗' },
      { label: 'Money Saved', value: `$${userStats.moneySaved}`, color: 'bg-black', icon: '💰' },
      { label: 'Next Booking', value: userStats.nextBooking, color: 'bg-black', icon: '⏰' }
    ],
    worker: [
      { label: 'Jobs Today', value: '8', color: 'bg-black', icon: '📋' },
      { label: 'Completed', value: '5', color: 'bg-black', icon: '✅' },
      { label: 'Earnings', value: '$320', color: 'bg-black', icon: '💰' },
      { label: 'Rating', value: '4.9', color: 'bg-black', icon: '⭐' }
    ]
  };
  ...
  return (
    <div className="min-h-screen bg-black text-[color:#00ddff]">
      {/* Header */}
          ...
          <div className="flex justify-between items-center py-4">
            <div className="flex items-center gap-3">
              <div className="w-10 h-10 rounded-2xl flex items-center justify-center bg-black shadow-lg">
                <span className="text-[color:#00ddff] font-bold text-xl">C</span>
              </div>
              <h1 className="text-2xl font-bold gradient-text">Caarvo</h1>
            </div>
            <div className="flex items-center gap-4">
              <div className="hidden sm:flex items-center gap-2">
                <div className="w-8 h-8 bg-[color:#00ddff] rounded-full flex items-center justify-center">
                  <span className="text-black font-bold text-sm">{user?.name?.charAt(0)}</span>
                </div>
                <span className="text-[color:#00ddff] font-medium">
                  {user?.name} <span className="text-azure">({user?.role})</span>
                </span>
              </div>
              <button
              ...
              <div className="card">
                ...
                <div className="space-y-3">
                  <div className="flex items-center gap-3 p-3 bg-black rounded-lg">
                    <div className="w-8 h-8 bg-black rounded-full flex items-center justify-center text-white text-sm">U</div>
                    <div>
                  ...
                  <div className="flex items-center gap-3 p-3 bg-black rounded-lg">
                    <div className="w-8 h-8 bg-black rounded-full flex items-center justify-center text-white text-sm">B</div>
                    <div>
                ...
                <div className="space-y-3">
                  {recentVehicles.length > 0 ? (
                    recentVehicles.map((vehicle: Vehicle, index: number) => (
                      <div key={vehicle._id} className="flex items-center gap-3 p-3 bg-black rounded-lg">
                    ...
                    <div className="text-center py-4">
                      ...
                      <button
                        ...
                        className="mt-2 text-[color:#00ddff] hover:text-[color:#00ddff]-dark"
                      >
                ...
                <div className="space-y-3">
                  {upcomingBookings.length > 0 ? (
                    upcomingBookings.map((booking: Booking) => (
                      <div key={booking._id} className="p-3 bg-black rounded-lg">
                        ...
                        <p className="text-sm text-[color:#00ddff] font-medium">{(booking.vehicle as any)?.make} {(booking.vehicle as any)?.model}</p>
                      </div>
                    ))
                  ) : (
                    <div className="text-center py-4">
          ...
          {user?.role === 'worker' && (
            <>
              <div className="card">
                ...
                <div className="space-y-3">
                  <div className="flex items-center gap-3 p-3 bg-black rounded-lg">
                    <div className="w-8 h-8 bg-black rounded-full flex items-center justify-center text-white text-sm">1</div>
                    <div>
