...
const PhonePeSimpleDemo: React.FC = () => {
  ...
  const createOrder = async () => {
    try {
      ...
      // Call the backend API to create a PhonePe order
      const response = await axios.post('/api/phonepe-sdk/create-order', {
        amount: amount / 100 // Convert to rupees (backend expects rupees, not paise)
      });
    ...
    <Container maxWidth="sm" sx={{ py: 4 }}>
      <Paper elevation={3} sx={{ p: 4, textAlign: 'center' }}>
        <img 
          ...
          alt="PhonePe Logo" 
          style={{ height: '40px', margin: '0 auto 20px' }} 
        />
        ...
        <Typography variant="caption" color="text.secondary">
          This is a demo implementation of PhonePe payment gateway.
        </Typography>
      </Paper>
    </Container>
  );
...
export default PhonePeSimpleDemo;